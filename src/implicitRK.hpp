...
namespace ASC_ode {
  ...
  // explicit runge-kutta for arbitrary Butcher tableaus with a lower triangular matrix for the a-coefficents
  ...
  public:
    ...
    void doStep(double tau, VectorView<double> y) override
    {
      ...
      {
        VectorView<> yi = m_y.range(i*m_n, (i+1)*m_n);
        yi = y;
        for (int j = 0; j < i; j++)
          yi += tau * m_a(i,j) * m_k.range(j*m_n, (j+1)*m_n);
        m_k.range(i*m_n, (i+1)*m_n) = 0.0;
        m_rhs->evaluate(yi, m_k.range(i*m_n, (i+1)*m_n));
      }
